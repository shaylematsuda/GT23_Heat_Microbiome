...
rack_pos <- read_csv("data/PAM_metadata.csv") %>%
  janitor::clean_names() %>%
...
# Join metadata
md <- rack_pos # %>%  # need frag metadata
```
...
pam <- left_join(pam5, md, by = c("date", "rack_number", "pos_number"))
```
...
ggplot(fvfm, aes(x = date, y = value)) +
  geom_violin() +
  geom_jitter(aes(color = factor(tank_number)))
